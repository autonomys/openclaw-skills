...
if [[ -z "$OUTPUT" ]]; then
  ...
  curl -sS --fail "$AD_DOWNLOAD_URL/downloads/$CID" ${AUTH_ARGS[@]+"${AUTH_ARGS[@]}"} 2>/dev/null \
    || curl -sS --fail "$GATEWAY/file/$CID"
else
  # Output to file — check HTTP codes for proper error reporting
  HTTP_CODE=$(download_to_file "$AD_DOWNLOAD_URL/downloads/$CID" "$OUTPUT" ${AUTH_ARGS[@]+"${AUTH_ARGS[@]}"})
